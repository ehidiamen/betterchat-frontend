...
const Home: React.FC = () => {
  ...
  const [newCharacter, setNewCharacter] = useState<CustomCharacter>({
    name: "",
    emoji: "",
    description: "",
  });
  ...
  useEffect(() => {
    if (user) {
      ...
      axios
        .get(
          `https://betterchat-backend.onrender.com/custom_character/${user.uid}`
        )
  ...
  const saveCustomCharacter = async () => {
    ...
    await axios
      .post(
        "https://betterchat-backend.onrender.com/custom_character",
        characterData
      )
  ...
  const sendMessage = async () => {
    ...
    const res = await axios.post(
      "https://betterchat-backend.onrender.com/chat",
      {
        ...
        <>
          ...
          {character === "custom" && customCharacter && (
            <p className="text-gray-600 font-semibold text-center">
              {customCharacter.emoji} {customCharacter.name} -{" "}
            ...
            <div className="bg-white p-4 rounded-lg shadow-md mb-4">
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                ...
                <input
                  ...
                  placeholder="Emoji"
                  onChange={(e) =>
                    setNewCharacter({ ...newCharacter, emoji: e.target.value })
                  }
                />
                <input
                  className="p-3 border-2 border-gray-400 rounded-lg text-lg font-semibold text-gray-900 placeholder-gray-600 focus:outline-none focus:border-blue-500"
              ...
              <button
                onClick={saveCustomCharacter}
                className="mt-3 w-full bg-green-500 hover:bg-green-600 text-white p-2 rounded-lg"
              >
                Save Custom AI
              </button>
            </div>
          ...
          <div className="flex">
            <input
              className="w-full p-3 border-2 border-gray-400 rounded-lg text-lg font-semibold text-gray-900 placeholder-gray-600 focus:outline-none focus:border-blue-500"
              value={input}
